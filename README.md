...
graph TD
    ...
    E -->|6. Safe Commands| F[(Docker Container<br/>Izole Test Ortamı)]
...
### Temel Özellikler
- **Idempotency**: Sistemin çökmesi durumunda aynı görev tekrar tekrar işlenmez (JSON tabanlı lock ve state yönetimi).
- **İzole Test Ortamı**: Yapay zekanın ürettiği komutlar, sizin bilgisayarınızda değil, izole ve sınırlı (CPU/Ram kotalı) bir Docker container'da çalışır.
- **Güvenlik Politikası**: `sudo`, `rm -rf /` gibi zararlı komutlar engellenir. Strict modda sadece `npm test`, `pytest`, `go test` vb. komutlara izin verilir.
...
2. [Docker](https://www.docker.com/) (Test executor için şart)
...
4. Bot, belirtilen SCM'e (Github/Gitlab) gidip o reponun `main` branch'indeki kaynak ve test dosyalarını akıllı bir filtreyle seçer (tokensize'i aşmamak için tüm repoyu değil, `*.ts, *.py` gibi gerekli dosyaları yükler).
5. İstek YZ (ör: Anthropic Claude 3.5 Sonnet) sistemine gönderilir. YZ'den **bir test ve patch senaryosu** oluşturması istenir.
6. YZ'nin ürettiği kod değişiklikleri kaydedilir.
7. O an boş ve geçici bir Docker Container (ör: `node:20-slim`) oluşturulur. İlgili repo klonlanır.
8. YZ'nin kodları basılır. Ve YZ'nin ürettiği test komutu (`npm run test`) çalıştırılır. Loglar dinlenir.
9. Eğer container'daki test **başarılıyla (Exit Code 0)** biterse, SCM'de projeden bir otomatik branch açılır, değişiklikler commit'lenir ve Pull Request oluşturulur.
10. Jira'daki o task'a geçici Docker limitleri, stderr, stdout logları ve ulaşılan başarı yüzdesi ile PR linki otomatik olarak yorum olarak yazılır, ticket Done'a taşınabilir.
