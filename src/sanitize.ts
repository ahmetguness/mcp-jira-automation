 ...
 * Does NOT invoke a shell — purely string splitting.
 *
 * @example tokenizeCommand('npm test -- --coverage') → ["npm", "test", "--", "--coverage"]
 * @example tokenizeCommand("echo 'hello world'") → ["echo", "hello world"]
 */
export function tokenizeCommand(cmd: string): string[] {
    ...
    if (tokens.length === 0) throw new Error("Command is empty after tokenization");
